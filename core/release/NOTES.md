...
- Let the dispatcher role start its workers with its own `scripts/spawn.sh`: the prompts `jaira roles install` ships now name the script where a worker is started, warn that calling `claude --permission-mode ...` yourself is refused as "Create Unsafe Agents", and say that `$HERDR_BIN_PATH` — not `command -v herdr` — is how to find Herdr on WSL. The script itself now opens a tab instead of splitting the pane, starts claude through `wsl.exe --cd` when Herdr runs on Windows so the worker no longer comes up in the Windows home in front of a trust dialog, branches with `feat/` (override with `JAIRA_BRANCH_PREFIX`) instead of `feature/`, and no longer writes another project's `COMPOSE_PROJECT_NAME` into your `.env`; re-run `jaira roles install --force` to pick it up.
- Stop opening pull requests from an agent session: the role prompts `jaira roles install` ships now say the agent pushes its branch and stops, and a person gives the command to open the pull request. `jaira-role-pr` still pushes to a pull request someone has opened and answers its review comments, and hands you a ready-made description to open it with; re-run `jaira roles install --force` to pick the new wording up.
...
- Ask `jaira whoami` where your tickets go: inside a board it now also prints the remote this board uses and where that name came from, the remotes the repository actually has, whether tickets travel on refs (with the reason when they do not), and how many tickets exist on this disk and on no ref. `--json` adds `board`, `remote`, `remote_source`, `remotes` (always a list, empty rather than null), `ref_mode`, `ref_mode_reason` and `file_only`. The remote it names is the one the ref commands actually use, and the reason is a sentence rather than a raw `gitref:` error. Outside a git repository it names no remote at all, in the text and in `--json` alike, because there is none to name.
...
- Read a card's left bar as three tags instead of one: its three cells now show the colours of the ticket's first, second and third tag, top to bottom in the order the tags stand on the ticket, so a board can be read along three axes at once; a cell with no tag of its own or whose tag has no line in `.jaira/tags` stays in the lane's shade, and tags past the third stay on the ticket and in `jaira show` but colour nothing.
- Expect a multi-line ticket title to read as one line on its board card: the break becomes a space instead of adding a fourth row to a three-row card, which used to push the cards under it out of the lane's row budget.
...
- Finishing a ticket files nothing on a board whose terminal lane was written by this version or later: it stays in `done` with everybody else's until you cut, `jaira logbook --all` puts the whole lane into today's folder, `jaira logbook <id>` still files one, `jaira logbook` alone still only lists, and a board holding more than ten finished tickets says so in its hint bar and files nothing on its own. On a board created before this change the terminal lane still carries `logbook-on-entry: true` in `.jaira/lanes/done.md` and still files on entry the moment a ticket arrives: remove that line by hand to stop it, once per checkout, because `.jaira/lanes/` is gitignored and the edit does not travel with the repository.
