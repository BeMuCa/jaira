...
func fieldFilled(t *ticket.Ticket, field string) bool {
	...
	case ticket.FieldTags:
		// A list, so "filled" is "has a member" — the same rule commits and the
		// plan follow. A lane that declares it produces tags is otherwise owed a
		// field the gate can never see satisfied.
		return len(t.Tags) > 0
	case "plan":
...
// DeclaredBy answers which installed lanes declare each producible field, in
// board order. It is what a renderer labels a FILLED value with, so it must
// not assert more than the frontmatter can back: the declaration is the only
// provenance the file carries — which lane actually wrote the latest value is
// the History's job once the schema grows one. That is why EVERY declarer is
// returned rather than the first: on a board where critique and review both
// produce review-summary, labelling a review-written value "(critique)" is a
// lie on the one screen where a person accepts work, while
// "(critique/review)" is true in every configuration.
