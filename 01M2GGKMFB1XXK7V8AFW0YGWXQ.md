...
updated-at: 2026-09-16T07:31:48Z
...
review-summary: "internal/cli/milestones.go:129 — when milestoneFiled (:362) finds the mark on the REF it returns the location string \"on its ref\", and that goes into refuseFiledInLogbook (:396), which prints \"has been filed into the logbook (on its ref) — 'jaira restore <name>.md' brings it back\". In that state this tree has no logbook copy — only the tree that filed it does — so the restore it names fails at core/ticket/store.go:537 with \"is not in the archive or in .jaira/logbook/\". That is exactly the defect round 6 named for logbook.go:283 and fixed there; the ref branch was never covered. Split the two sources: have milestoneFiled report WHICH one it found, and for the ref case use refuseFiledOnDisk's wording ('jaira restore <name>.md' in the tree that filed it) instead of refuseFiledInLogbook's — the helper's own doc comment at :391 already claims 'the file is not on disk any more because this very tree filed it', which the ref case contradicts. Same at :244 (add/rm), which takes the same branch."
...
- [ ] refuseFiledOnRef als dritte Weigerung neben OnDisk/InLogbook: gemeinsamer Kern, weil beide auf 'jaira restore <name>.md' IN DEM BAUM, DER ABGELEGT HAT zeigen - InLogbook bleibt der einzige Fall, in dem restore hier laeuft
