...
updated-at: 2026-09-18T07:53:43Z
...
review-summary: "core/gitrepo/git.go:194 liest 'git ls-files --others' ohne core.quotePath abzuschalten: ein untracked Pfad mit Nicht-ASCII-Zeichen kommt als C-Zitat zurueck (\"\\303\\204nderung.txt\"), und die Zeile 206 'git diff --no-index -- /dev/null <pfad>' findet diese Datei dann nicht. Sie meldet das mit Exit 1 - genau dem Code, den runTolerating(1, ...) als Normalfall durchwinkt - also ist err nil, out leer, und die Datei faellt still aus dem Payload, waehrend commits_source weiter '+worktree' sagt und complete true bleibt. Nachgestellt: git init, 'Aenderung.txt' mit Umlaut untracked -> ls-files gibt \"\\303\\204nderung.txt\", diff --no-index darauf sagt 'error: Could not access', Exit 1. Das ist die Bruchstueck-als-vollstaendig-Meldung, die dieses Ticket abschafft, nur an der neuen Stelle. Fix: ls-files mit -z aufrufen und auf \"\\x00\" splitten statt auf \"\\n\" (-z schaltet das Zitieren aus), oder ersatzweise r.run(\"-c\", \"core.quotePath=false\", \"ls-files\", ...). Zusaetzlich sollte der --no-index-Aufruf einen echten Fehler nicht mit dem Normalfall teilen - Exit 1 heisst dort sowohl 'die Dateien unterscheiden sich' als auch 'Pfad nicht lesbar'; mit leerem out und nicht leerer Datei ist das ein Fehler, kein Treffer"
