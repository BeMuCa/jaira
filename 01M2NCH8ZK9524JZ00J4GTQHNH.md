...
outcome-what: "Die sechs offenen DoD-Punkte 7-12 gebaut, alle in Prompts und Board-Konfiguration, kein Go-Code. jaira-dispatcher/SKILL.md bekommt den Abschnitt 'In conversational mode, a critique runs beside the work': im Gespraechsmodus laeuft ein zweiter, NUR LESENDER Kritiker gleichzeitig mit dem implementierenden Worker, gibt jeden Befund sofort weiter, und genau eine Stelle — der Dispatcher — schreibt review-summary und macht den einen 'jaira move'. Der '--no-worktree'-Absatz benennt diesen Kritiker als die eine Ausnahme vom Verbot, zwei Worker in ein Verzeichnis zu setzen. Der Halbsatz 'Testing is not a lane' ist gestrichen; die Zeile lautet jetzt '/jaira-role-lane <id> <lane> — every lane, testing included', spawn.sh und jaira-role-tester sind unangetastet. jaira-role-lane/SKILL.md laesst die Pause nach jedem DoD-Punkt 'git status --short' neben 'git diff' laufen und legt den Inhalt jeder '??'-Datei mit vor. .jaira/lanes/critique.md fuehrt 'notes' in input-requires und sagt im Prompt, die Notizen vor dem Diff zu lesen. Drei Zeilen in core/release/NOTES.md unter ## Unreleased."
outcome-why: "Alex hat das Ticket am 17.09. aus signoff zurueckgeholt, weil 'jaira move --to done' die Punkte 7-11 als offen gemeldet hat. Die beiden inhaltlichen Anlaesse stehen auf dem Ticket: die critique-Lane laeuft heute erst NACH der implementierenden, und dieses Ticket hat so acht Runden gebraucht, deren teuerste (resume trug das neue Feld nicht) in Runde sieben kam — bei einer Abbruchregel von drei Runden waere sie nie gefunden worden. Und 'git diff' zeigt keine untracked Dateien, womit ein DoD-Punkt aus einer neuen Datei im Gespraechsmodus ohne Pause durchlief — genau das, was der Modus verhindern soll; die Implementierung dieses Tickets ist selbst das Beispiel (core/validate/mode_test.go, internal/cli/mode_test.go sind neu)."
outcome-resolves: "DoD 7: SKILL.md:100-136, im selben Prompt wie der Modus. DoD 8: .jaira/lanes/critique.md:10 plus Prompt-Absatz, nachgestellt an diesem Ticket ('--for-lane critique --json' liefert missing=null und die acht Runden als notes). DoD 9: SKILL.md:127. DoD 10: jaira-role-lane/SKILL.md:50-80, nachgestellt (angelegte probe_neu.go: 'git diff' 0 Zeilen, 'git status --short' '?? probe_neu.go'). DoD 11: grep findet den Halbsatz nicht mehr, spawn.sh und jaira-role-tester stehen nicht in 'git status'. DoD 12: NOTES.md:16-18. Alle zwoelf DoD-Punkte sind jetzt [x]; go build, go vet und 'go test ./... -count=1' sind gruen."
review-summary: |-
  core/role/builtin/jaira-dispatcher/SKILL.md:115-117 — the running critique's read-only-ness lives only in the line the dispatcher types when starting the worker ('Say that in the line you start it with, because its lane prompt tells it to do all three'). That is channel B from this ticket's own brainstorm decision, rejected there because it dies with the session: core/role/builtin/jaira-role-lane/SKILL.md is what a restarted or compacted worker re-reads, and it still tells it to 'jaira note', write review-summary and 'jaira move', with a Boundaries section calling the lane the deliverable. Put the clause where the worker reads it instead — a third item in the 'Two things change' list at jaira-role-lane/SKILL.md:48: in mode: conversational, a worker whose lane argument is not the ticket's current status ('jaira show <id> --json') is the running critique — report findings to the dispatcher, write nothing: no note, no set, no move. The discriminator works both ways, since the ordinary critique lane runs with status=critique and keeps writing.
  core/release/NOTES.md:18 — 'no lane shipped with jaira asked for it' is false: core/lane/builtin/20-in-progress.md:9 has notes in its input-requires and ships with jaira. Drop the clause, or replace it with the true one — the in-progress lane already asks for it, critique is not a shipped lane at all (the note of 2026-09-17 18:19 establishes that), which is why this reaches a reader's board only by hand.
review-gaps: |-
...
- [x] Genau eine Stelle schreibt review-summary und bewegt das Ticket. Laufen mehrere Kritiker gleichzeitig, lesen sie nur; das Zusammenfuehren und der eine 'jaira move' gehoeren dem Dispatcher.
  proof: core/role/builtin/jaira-dispatcher/SKILL.md:130 ('Exactly one place writes review-summary and moves the ticket, and it is you') plus core/role/builtin/jaira-role-lane/SKILL.md:50-72 — der mitlaufende Kritiker erkennt sich selbst am Ticket ('jaira show --json': lane-Argument != status) und schreibt nichts, also ueberlebt die Nur-Lese-Regel auch einen Neustart des Workers
