...
updated-at: 2026-09-14T20:04:24Z
assignee: ""
...
- **2026-09-14 20:04 · Alexander Sacharov** — Am 2026-09-14 vorgezogen und sofort in den INSTALLIERTEN Kopien unter ~/.claude/skills gemacht, weil das Starten von Workern nicht mehr brauchbar war. Geaendert wurden drei Stellen, alle ausserhalb des Repositories:

1. jaira-dispatcher/SKILL.md, Abschnitt Transport: scripts/spawn.sh ist jetzt DIE Anweisung, nicht eine Fussnote. Mit den beiden Gruenden dazu - 'claude --permission-mode acceptEdits' selbst aufzurufen wird als 'Create Unsafe Agents' abgelehnt, und 'command -v herdr' beantwortet auf WSL die falsche Frage, weil die Datei herdr.exe heisst und unter /mnt/c liegt.
2. jaira-teamlead/SKILL.md, dieselbe Anweisung fuer das Starten eines Dispatchers.
3. scripts/spawn.sh: ein Tab je Worker statt eines Splits (vorher schon gemacht, siehe Notiz davor).

Der Grund fuer die Eile steht in den Zahlen: von drei Dispatchern des Tages bekamen zwei keinen einzigen Worker in einen Tab, und zuletzt lief ueberhaupt kein Worker mehr in einem eigenen Tab - 'herdr tab list' zeigte kein einziges Label der Form <ticket>/<lane>.

Damit steht dieses Ticket vor einer Besonderheit: seine Arbeit ist auf diesem Rechner bereits getan, aber am falschen Ort. Es bleibt trotzdem zu tun, und zwar vollstaendig: dieselben Aenderungen gehoeren nach core/role/builtin, sonst ueberschreibt der naechste 'jaira roles install --global --force' sie, und kein Teamkollege bekommt etwas davon. Die installierten Fassungen sind ab jetzt die Vorlage - uebernehmen, nicht neu erfinden. Sicherungen liegen als spawn.sh.bak daneben; fuer die beiden SKILL.md gibt es keine, dort ist der Diff gegen core/role/builtin die Wahrheit.
