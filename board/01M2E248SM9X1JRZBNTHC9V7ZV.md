...
- **2026-09-13 22:06 · Alexander Sacharov** — review: angenommen, kein Rueckversand. Von Hand nachgestellt statt nur gelesen - Binary gebaut und gegen ein Ersatz-HOME und einen frischen --into-Ordner durchgespielt: Erstlauf 8 written, zweiter Lauf 8 unchanged Exit 0, handgeaenderte Datei skipped Exit 3 und woertlich erhalten, --force overwritten Exit 0, spawn.sh 755, --project ohne Board Exit 1, alle drei Usage-Zweige Exit 2, --json-Fehler auf stderr. Die Modus-Reparatur steht wirklich auf beiden Zweigen von writeFile (install.go:80 und :92), nicht nur in der Erzaehlung.
Zwei Dinge fuer spaeter, bewusst nicht in dieser Lane repariert und jeweils ein eigenes kleines Ticket wert:
- 'roles install --project' geht durch openStore() und verlangt damit ein .jaira, obwohl es nur den Repository-Wurzelpfad braucht. Ohne Board: Exit 1 mit 'run jaira init' - eine Meldung, die mit dem Installieren von Prompts nichts zu tun hat. Ein git-Wurzelpfad wuerde reichen.
- core/role/role_test.go:104: der Wachposten auf '/jaira' kann fuer die Form, die sein Kommentar nennt, nie greifen - in '/jaira-role-lane' steht vor 'role-lane' ein Bindestrich, strings.Index findet das blanke '/role-lane' dort also gar nicht. Der Test faengt den Fall aus Runde 5 trotzdem (mit 'hand it to /jaira-role-lane, not to /role-tester' schlaegt er fehl, geprueft); falsch ist nur der Kommentar.
Nicht geprueft, weil nicht pruefbar: ob der Inhalt der sieben Prompts stimmt. Die Tests halten nur name: == Ordnername und das Praefix jedes Querverweises.
