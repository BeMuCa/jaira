...
related: []
commits:
  - 18f8ace97c91418069503f079a4d68c89a0aad50
  - ab15a124b1352ec2e7b8cc90cd34fb4af4bfc91c
  - f4e7bbf6a67db1faf89f43aadd1e1fb31d405029
  - fa6a7a93bcb41be904b9b8e64c011b4c7211e931
  - fbfaea1347bf2adc4758f4a7691f8b44d553b278
  - 3f0c8bf38ea2f302ccdfe7ebc462df927636eff9
  - 64f43bf590cfa0b56bd27756d4125fdd5402385d
  - 9f336da90c1a1f5deffea200dc8a53d3ac0c4de9
  - 61a889d3dcaab486dab255a13ffaec43b4490da2
  - 3d59c7c73c69df61cffcbb7391b1ac5bfc401d73
  - 6fc54254541e7d82fe67bbd3bb2a256c67206594
  - 5c642ca836f996f852db2aef167ca36d37d2462b
  - 5e9bf2e9f16a532697f2ce321fb77bc264c0693c
  - 946dca6b688a45e80a78d6110bb4b86a9c002c84
  - 1837dfe43ad0ac1a5be959d790692d08de82a245
  - 152725b5d55d5d44e37deb305421df1b75902f6a
...
outcome-what: "Reviewed the wintrap diff against the definition of done and re-ran the four verification commands rather than trusting the notes: go test ./... -race, go vet ./..., GOOS=windows GOARCH=amd64 go vet ./... and GOOS=windows GOARCH=amd64 go build ./cmd/jaira all exit 0. Every DoD item holds: five rules each with a firing fixture and the remedy in the failure text, tree green without a loosened rule, ci.yaml:33-40, README.md:826-827, no NOTES.md line. All three critique findings are genuinely fixed in the shipped code. Four review outputs written."
outcome-why: "Held the checker against the seven historical Windows fixes end to end for the first time, which is the real test of the feature and which nobody had run: scanning the tree as it stood before each fix, it names five of the seven on the exact line. That is the evidence the ticket had been carrying as an assertion. It also shows the ticket overstates itself - two of the seven are caught by nothing, so \"all seven were these five patterns\" is five of seven, and a person signing off should see the corrected number."
outcome-resolves: "Accept with three gaps, none of them a defect in the shipped diff and none worth another in-progress round: (1) the context and outcome-why claim all seven historical fixes were the five patterns - it is five of seven, missing 4d3cace (filepath.Join on an io/fs path, the mirror image of rule 5, a total outage at the time) and 29ae82a (os.SameFile identity semantics, a sixth pattern); a rule 6 is a follow-up ticket. (2) //wintrap:ok is not rule-scoped - measured: a comment whose reason is a rule 5 one silences a rule 4 finding on the next line; harmless today, all four exemption sites carry a correct reason. (3) go doc ./internal/wintrap now says \"no source-code package in directory\" after the move to _test.go, so the package comment is no longer discoverable; README stays literally true. Not verified on a real Windows machine."
...
review-check: "Do these in a terminal in a checkout of branch feat/9ET6NC-per-board-remote. Each step says what you should see; if you see something else, that step failed.\n\n1. Run: go test ./internal/wintrap -v\n   You should see six tests, all PASS, in well under a second: TestEachPatternFires, TestCoveredEmbedIsSilent, TestExemptionIsHonoured, TestRepositoryIsClean, TestMatchPattern, TestNonTextEmbedIsNotExempt.\n\n2. Now break the repository on purpose and watch it get caught. Open core/gate/gate_test.go and go to line 28. It reads:\n       if err := os.WriteFile(filepath.Join(dir, filename), []byte(body), 0o644); err != nil {\n   Change just the path part so it is glued together with a slash instead:\n       if err := os.WriteFile(dir+\"/\"+filename, []byte(body), 0o644); err != nil {\n\n3. Run: go test ./internal/wintrap -run TestRepositoryIsClean\n   You should see FAIL, and in the output a line reading \"core/gate/gate_test.go:28: windows trap 5: a path is glued together with a literal \\\"/\\\" and handed to os.WriteFile\", followed by a line starting \"fix:\" telling you to build it with filepath.Join. The thing to check is that second line: the failure names the remedy, not only the site.\n\n4. Undo it: git checkout core/gate/gate_test.go — then run go test ./internal/wintrap again. Back to PASS.\n\n5. Now the escape hatch. Make the same change to line 28 again, and this time put a line reading\n       //wintrap:ok\n   directly above it. Run: go test ./internal/wintrap -run TestRepositoryIsClean\n   It should PASS — the comment silences that one line, and the rule itself is untouched. Undo everything: git checkout core/gate/gate_test.go\n\n6. Check the Windows cross-build by hand, the two lines README.md:826-827 tells you to run before pushing:\n       GOOS=windows GOARCH=amd64 go vet ./...\n       GOOS=windows GOARCH=amd64 go build ./cmd/jaira\n   Both should print nothing and exit 0. Nothing printed means nothing is wrong.\n\n7. Prove step 6 can actually fail. Create a file internal/cli/probe.go containing exactly:\n       package cli\n       import \"syscall\"\n       func probe() { syscall.Umask(0) }\n   Run GOOS=windows GOARCH=amd64 go vet ./... again. It should now fail with \"undefined: syscall.Umask\" — that function exists on Linux and not on Windows, which is the class of breakage this CI step is for. Plain go vet ./... on Linux stays silent, which is the whole point. Then delete internal/cli/probe.go.\n\n8. Read .github/workflows/ci.yaml lines 29 to 40. Confirm the step is named \"Cross-check the Windows build\", is guarded by if: matrix.os == 'ubuntu-latest' so it runs once rather than on all three runners, and runs the same two commands you ran in step 6.\n\n9. Confirm nothing was added for this ticket to core/release/NOTES.md: run\n       git log --oneline 5c642ca^1..1837dfe -- core/release/NOTES.md\n   Every commit listed should name S1VM40, not APABM4. This ticket is test and CI only, which a user cannot observe from outside the binary.\n\n10. The one thing this flow cannot check is Windows itself — there is no Windows machine in it. The evidence that these five patterns really do break there is the historical fix commits: run git log --oneline -i --grep=windows, and each of those fixes records the windows-latest failure it repaired. I checked separately that the checker, run against the tree as it stood before each of the seven, names five of them on the exact line and misses two — see review-gaps."
---
