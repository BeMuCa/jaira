...
updated-at: 2026-09-17T18:28:04Z
updated-by: Alexander Sacharov
claimed-by: DESKTOP-RFTCH11-12830
claimed-at: 2026-09-17T18:13:16Z
outcome-what: "Die beiden Befunde aus critique-Runde 9 repariert. core/role/builtin/jaira-role-lane/SKILL.md: die Liste im Abschnitt 'Is this ticket in conversational mode?' hat einen dritten Punkt und er steht als erster — ein Worker liest im Modus das Ticket ('jaira show <id> --json') und erkennt an einem lane-Argument, das nicht dem status entspricht, dass er die mitlaufende Kritik ist: dann liest er nur, meldet jeden Befund sofort an den Dispatcher und schreibt nichts (keine note, kein set, kein move, kein review-summary, keine Commit-Zeile). 'Show the code' und 'Do not commit' sind dadurch Punkt 2 und 3. core/role/builtin/jaira-dispatcher/SKILL.md Schritt 2 begruendet die Nur-Lese-Regel jetzt mit dem Worker-Prompt statt mit der Startzeile und verlangt die Startzeile zusaetzlich. core/release/NOTES.md: die falsche Zeile 'no lane shipped with jaira asked for it' nennt jetzt die ausgelieferte in-progress-Lane und den wahren Grund (critique ist keine ausgelieferte Lane), und die Zeile zur mitlaufenden Kritik nennt den Unterscheider und den neuen jaira-role-lane-Prompt."
outcome-why: "Die Nur-Lese-Regel des mitlaufenden Kritikers lebte ausschliesslich in der Zeile, mit der der Dispatcher ihn startet — das ist Kanal B aus der Brainstorm-Entscheidung dieses Tickets, dort verworfen, weil er mit der Sitzung stirbt. Ein neu gestarteter oder komprimierter Worker liest jaira-role-lane/SKILL.md wieder und fand dort unveraendert die Anweisung, note, review-summary und move zu schreiben — also genau die zwei Schreiber auf einem Feld, die derselbe Prompt zwanzig Zeilen weiter verbietet. Und die NOTES.md-Zeile behauptete etwas, das core/lane/builtin/20-in-progress.md:9 widerlegt."
outcome-resolves: "DoD 9 (genau eine Stelle schreibt review-summary und bewegt das Ticket): jaira-role-lane/SKILL.md:50-72 traegt die Regel jetzt dort, wo der Worker sie wiederliest, statt nur in der fluechtigen Startzeile — --proof nachgezogen. DoD 4 und 5: unveraendert erfuellt, ihre --proof-Zeilen zeigen auf die verschobenen Zeilennummern (SKILL.md:107-128 bzw. 74-105). DoD 12 (je eine NOTES.md-Zeile): die Zeile zur mitlaufenden Kritik ist korrigiert und nennt beide neuen Prompts. Plan 24 und 25 [x]; go build ./... und go test ./core/... ./internal/... -count=1 gruen."
review-summary: |-
