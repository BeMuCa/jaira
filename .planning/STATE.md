...
| Date | Task | Outcome |
|------|------|---------|
| 2026-08-21 | 260821-u7h | The board was already windowing — `renderBoard` fits `m.width/(minColWidth+2)` columns, so ten lanes at 125 columns showed five, not ten squeezed into twelve cells each. The two real faults were that it never said so, and that the window was derived statelessly from the cursor, pinning it to the right edge so every step past the fold shifted every column. `z` now hides lanes holding nothing, saying how many it hid — a reader who cannot see `blocked` must not conclude nothing is blocked — and the window became state with a margin, `scrolloff = min(2, (perScreen-1)/2)`, that keeps two lanes either side of the active one and clamps at the ends rather than scrolling past them or padding with blanks. At five columns that is exactly centring; the columns-hold-still benefit only appears from six up, which is why the steady-state test sits at 170 columns. Hiding runs first, the window applies to what remains, both in one `boardFit`. A wide terminal renders byte-for-byte as before: the notice is appended only when something is hidden or off-screen, above the version line, which stays last ([260821-u7h](./quick/260821-u7h-narrow-terminal-board-hide-empty-lanes-a/)) |
